...
- 다른거 하지 말고 최우선 순위로 딱 여기까지만 선 그어놓고 이 안에서 하자
	...
	- [ ] 취약알고리즘 공략 적어놓은거. 골드3이하 혹은 레벨3이하인거 다 풀기
...
- 프로그래머스 레벨3 다시 풀 문제 (2회차)
	- 경주로 건설, 다단계 칫솔 판매, 표 편집, 파괴되지 않은 건물
- 랜덤 디펜스 후 취약 알고리즘 공략
...
### STL
- list erase
	- li.erase(iter); 하면, 현재 iter가 가리키는 요소를 삭제해줌.
	- 이때, iter는 여전히 삭제된 요소를 가리키고 있게 되는데, 해당 요소는 list에서 제거되었기 때문에 iter++, iter-- 같은 걸 하면 예상치 못한 결과를 얻게됨.
	- 따라서, iter = li.erase(iter); 라고 처리함으로서 iter가 예상되게끔 만들어주는게 좋음. 계속 iter를 사용할 예정이라면.
	- li.erase(iter)는 마지막으로 삭제한 요소의 다음 요소를 가리키는 반복자를 반환함.
	- 다만, 마지막으로 삭제한 요소가 리스트에서 마지막 요소였을 경우 end()를 반환함.
	- 그러나, 헷갈릴 수 있는 부분이 뭐냐면 마지막 요소를 삭제하고 반환한 반복자를 현 반복자에 덮어씌울 경우 현 반복자는 li.end()와 동일하지만 (*iter)를 출력해보면 제거된 요소를 빼고서 리스트의 마지막 요소를 가리키는것 처럼 나옴! 즉, iter == li.end() 인데도 불구하고 cout << (*iter) 를 하면 값을 내뱉음... 아마, 컴파일러나 버전마다 다른 결과를 얻을 수도 있음.
	- 그래서 이런 말도 안되는 결과가 나옴.
		```
		list<int> li = {1,2,3,4,5};
		auto iter = li.end();
		iter--; // 5를 가리킴. 마지막 요소.
		iter = li.erase(iter); // 얘는 지금 end를 가리킴.
		if(iter == li.end()) cout << "Your right" << '\n'; // 출력함
		cout << (*iter) << '\n'; // 4 출력
		iter--;
		cout << (*iter) << '\n'; // 4 출력
		```
	- 그러나 그냥 헷갈릴 수 있는 요소를 배제하고서,  마지막으로 삭제한 요소의 다음 요소를 가리키는 반복자를 반환한다는것만 기억해두는게 나을듯.
- 반복자에 대한 고찰
	- vector에서 새로운 요소를 추가하게 되면, vector 컨테이너 전체가 처음 주어진 capacity를 초과하게 되면 메모리 재할당을 하게 되고 이때 vector의 각 요소들을 가리키고 있는 반복자가 길을 잃을 수 있음.
	- vector의 요소들을 가리키는 반복자들을 다른 컨테이너에 잘 담아놓아도, vector에 새로운 요소를 추가하면 vector가 메모리 재할당을 하게 되고 해당 요소들은 다른 메모리 영역에 저장되고 기존의 메모리 영역은 삭제됨으로서, 요소들을 가리키고 있던 반복자들이 엉뚱한 값을 가리키게 되는거야.
	- 이걸 Undefined Behavior(정의되지 않은 동작) 이라고 함.
	- 그래서 내가 처음에 프로그래머스의 '표 편집' 이라는 문제. 이게 뭐가 문제였냐면, 되돌리기를 위해서 스택에다가 반복자와 요소의 값을 넣어둔거야. 근데 일단 요소를 삭제하는 순간 해당 반복자들은 길을 잃게 되니까, 어떤 상황에서는 내가 원하는 결과가 나오고 어떤 상황에서는 아닌 결과가 나오게 되는거임..
	- 그런데 list의 경우 linkedList 기반으로 구현된 컨테이너인데, 더 정확히 말하면 양방향연결리스트 기반으로 구현된거라 새로운 요소를 더한다 해도 메모리 재할당이 발생하지 않음. 즉 새로운 요소를 더한다 해도 그밖의 다른 요소들을 가리키고 있던 반복자들은 계속 정확히 처음 가리키던 요소들을 가리키고 있음이 보장되는거임.
- 이분탐색
...
- 수학
	...
	- 최소공배수
		- LCM(a,b) == a*b / GCD(a,b)
	- 소수
		- 소수인지 아닌지 순차적으로 검사할때, i*i < num 까지만 검사하면 됨.
...
- 게임이론
	- dp로 푸는게 정석인데, 홀수냐 짝수냐에 따라 결과가 정해지거나, 일단 테이블을 채우다 보면 패턴이 보이기도 함.
- 그리디
...
- 투 포인터
  - 2중for문으로 간단히 풀릴 수 있을것 같은데, 시간초과날것 같은 문제를 투 포인터를 사용하여 반번의 반복문으로 해결 가능
  - 하나의 정렬된 배열에서 두개의 포인트를 잡고 두 포인트가 가리키는 값을 피연산자로 두어 연산을 한 값이 문제에서 주어진 기준에 적합하냐 아니냐에 따라 포인트를 조정함
  - 보통 left 포인터와 right 포인터를 잡고 이를 배열 양 끝에서 시작해서 서로 가까워 지거나, 배열 중앙에서 시작해서 서로 멀어짐.
  - 어느때 left 포인터를 움직일지 어느때 right 포인터를 움직일지를 파악하는게 문제의 핵심

- 슬라이딩 윈도우
  - 투 포인터랑 거의 비슷한데 얘네는 포인터 두개 st와 en를 잡고 index 0에서 둘다 시작함.
  - en가 오른쪽으로 한칸씩 움직이면서 st와 en 사이의 배열들의 연산 값을 가지고 특정 기준에 맞는지 안맞는지에 따라 en가 이동하거나 st이 이동함. 어떤애가 이동하는지가 역시 핵심.
  - st나 en가 이동할떄마다 st와 en 사이의 요소들을 가지고 연산한 값을 새로 연산하면 사실상 2중 for문 돌리는거나 다름이 없으니, 
  - en이 앞으로 가면 해당 결과에 값을 추가 하고, st이 앞으로 가면 해당 결과에 값을 빼는 등의 약간 누적합 비슷한 개념이 들어감.
  - 투포인터도 마찬가지인데, 조건에 부합할 경우 답으로 리턴할 문자열을 하나 선언해두고 이를 계속 갱신하는 방식은 문자열을 새롭게 계속 복사해서 메모리를 사용하는 상황이 되버릴 수 있음.
  - 따라서 문자열을 계속 갱신하지 말고, 문자열을 나타낼 수 있는 시작 idx와 길이 정도를 갱신해두는것이 좋음.
  - answer = s.substr(st, len); 이 아니라 minLen = len; minSt = st; 이런식으로 반복문안에서 갱신하고, 반복문 밖에서 마지막으로 s.substr(minSt, minLength); 하면 메모리 사용 아낄 수 있음.
