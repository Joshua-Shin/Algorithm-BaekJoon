...
- DP
  - **풀이법**
	  1. 테이블을 정의한다. ex) d[x] = x번째 계단까지 올랐을때 얻을 수 있는 최댓값
	  2. 점화식을 세운다. ex) d[x] = max(d[x-1], d[x-2]) + cost[x];
	  3. 초기값을 설정한다. ex) d[1] = cost[1], d[2] = cost[2];
	  4. for문을 돌린다.
  - Top-Down 도저히 안되겠고 Bottom-up으로 풀자.
