...
- 반복자에 대한 고찰
	- 일단, 반복자는 list든 vector든 상관없이 해당 반복자가 가리키고 있는 요소가 포함된 컨테이너에서 insert나 erase가 발생하면 그 요소와 관계가 없는 다른 반복자들도 메모리 재할당의 이유로 길을 잃을 수 있어 -> 잘못된듯. vector는 메모리재할당을 하지만 list는 그렇지 않기 때문에 list의 요소들을 가리키는 반복자들은 list에 요소가 추가되거나 삭제되어도 계속 같은 요소를 가리키는것이 보장됨.
	- 정확히 말하면 길을 잃을수도 있고 아닐수도 있어. 때문에 같은 컴파일러에 같은 버전에 온전히 똑같은 코드더라도 이후의 값이 다르게 나타날 수 있는거야.
	- 이걸 Undefined Behavior(정의되지 않은 동작) 이라고 함.
	- 때문에, 반복자는 insert를 하든 erase를 하든 꼭 반환값을 현재의 반복자에 덮어씌워서 이후 사용 가능하게 만들고, 또한 현재의 반복자뿐만 아니라 다른곳에 놓인 또 다른 반복자도 길을 잃지 않으려면 다시 위치를 잡아줘야돼.
