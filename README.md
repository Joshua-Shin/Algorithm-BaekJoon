...
- 테스트 1주일 전 : 테스트 조건(ex: 외부 ide 금지, 구글링 금지)에 맞춰 준비. 프로그래머스에서 안푼 level 2,3 문제 혹은 구현 문제 위주로 풀어
- 테스트 3일 전 : 바킹독 문제집에서 단원별로 대표문제 한두문제씩 풀어.
- 테스트 하루 전 : 괜히 새로운 문제 막 풀지말고, 여기 readme 파일 다시 차근히 보고 좀 아리까리한 유형은 조금 더 기본문제 풀어.
- 풀이법을 봐도 모르겠는 경우 
